...
class makesnr:
    ...
    def __init__(self, dwilist, noisepath=None, maskPath=None):
        ...
        nDWI = len(dwilist)     # Number of input DWIs
        # Open the first image in list
        hdr = nib.load(dwilist[0])
        sDWI = hdr.shape        # Shape of input DWIs
        nvox = sDWI[0] * sDWI[1] * sDWI[2]
        if hdr.ndim != 4:
