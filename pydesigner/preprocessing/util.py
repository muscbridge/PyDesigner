...
def find_valid_ext(pathname):
    ...
    valid_extensions = (".nii.gz", ".nii", ".json", ".bval", ".bvec")
...
def json2fslgrad(path):
    ...
    if (image.getBVAL() is None) or (image.getBVEC() is None):
        if not image.hasJSON():
            raise Exception(
                "It is not advisable to run multi-series "
        ...
        if nDWI <= 15:
            ...
            np.savetxt(
                op.join(image.getPath(), image.getName() + ".bval"),
                np.c_[bval],
                delimiter=" ",
                fmt="%d",
            )
            ...
            raise Exception(
                "PyDesigner currently only supports "
...
class DWIParser:
    ...
    def __init__(self, path):
        ...
        if self.InputType.size > 1:
            raise IOError(
                "It appears that multiple types of files "
                "have been parsed as input. Please input "
                "a specific type of input exclusively. "
                "Detected inputs were: {}".format(self.InputType)
            )
