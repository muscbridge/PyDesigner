...
class modelmrtrix(BaseModel):
    ...
    def file_input_valid(cls, var: str, info: ValidationInfo):
        ...
        if not isinstance(var, str):
            raise TypeError(
                f"Input file path {info.field_name}: {var} is not a string "
                f"type. Type entered is {type(var)}."
            )
        if not op.exists(var):
            raise FileNotFoundError(
                f"Input file {info.field_name}: {var} does not exist."
            )
    ...
    def file_output_valid(cls, var: str, info: ValidationInfo):
        ...
        if not isinstance(var, str):
            raise TypeError(
                f"Output file path {info.field_name}: {var} is not a string "
                f"type. Type entered is {type(var)}."
            )
    ...
    def correct_nthreads(cls, var: int, info: ValidationInfo) -> int:
        ...
        if not isinstance(var, int):
            raise TypeError(
                f"{info.field_name} is not provided as a {type(var)}. Please "
                "provide a positive integer."
            )
        if var <= 0:
            raise ValueError(
                f"{info.field_name} needs to be a valid integer."
            )
