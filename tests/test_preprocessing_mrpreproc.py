...
def test_miftonii_output_failure(tmp_path):
    ...
    with patch("subprocess.run") as mock_subprocess:
        mock_subprocess.return_value = MagicMock(returncode=1, stderr="stderr")
...
def test_miftonii_output_success(tmp_path, nthreads, force, verbose):
    ...
    output_bvec = str(tmp_path / "output.bvec")
    mrpreproc.miftonii(
        PATH_MIF, output_nii, nthreads=nthreads, force=force, verbose=verbose
    )
...
def test_niitomif_failure_bval(tmp_path):
    ...
    assert (
        f"Input file path ({os.path.splitext(PATH_DWI_NO_BVAL)[0]}.bval) does not exist"
        in str(exc.value)
    )
...
def test_niitomif_failure_bvec(tmp_path):
    ...
    assert (
        f"Input file path ({os.path.splitext(PATH_DWI_NO_BVEC)[0]}.bvec) does not exist"
        in str(exc.value)
    )
...
def test_niitomif_failure_json(tmp_path):
    ...
    assert (
        f"Input file path ({os.path.splitext(PATH_DWI_NO_JSON)[0]}.json) does not exist"
        in str(exc.value)
    )
...
def test_niitomif_success(tmp_path, nthreads, force, verbose):
    ...
    output_mif = str(tmp_path / "output.mif")
    mrpreproc.niitomif(
        PATH_DWI, output_mif, nthreads=nthreads, force=force, verbose=verbose
    )
...
def test_denoise_noisemap_invalid(tmp_path):
    ...
    assert "Please specify whether noisemap generation is True or False" in str(
        exc.value
    )
...
def test_degibbs_output_success(tmp_path, nthreads, force, verbose):
    ...
    output_mif = str(tmp_path / "output.mif")
    mrpreproc.degibbs(
        PATH_MIF, output_mif, nthreads=nthreads, force=force, verbose=verbose
    )
    assert os.path.exists(output_mif)
    assert mrinfoutil.format(output_mif) == "MRtrix"
    mrinfoutil.size(output_mif) == (2, 2, 2, 337)
...
def test_undistort_output_failure_conversion(tmp_path):
    ...
    with patch("subprocess.run") as mock_subprocess:
        ...
        assert "Extraction of FSL BVEC and BVAL gradients failed" in str(exc.value)
        assert "stderr" in str(exc.value)
